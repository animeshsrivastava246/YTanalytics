...
export default function ChannelDetailScreen() {
  ...
  const { data: channel, isLoading, isError } = useChannel(id as string);
  ...
  if (isError || !channel) {
    return (
      <View style={[styles.container, styles.center]}>
        <AppText variant="h3" color="error">Error loading channel.</AppText>
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: insets.bottom + 40 }}>
      ...
      <View style={styles.content}>
        <AppText variant="h1" style={styles.title}>{channel.title}</AppText>
...
const styles = StyleSheet.create({
  ...
  heroContainer: { width: '100%', height: 220, position: 'relative', justifyContent: 'flex-end', alignItems: 'center', paddingBottom: 20 },
  heroImageBlur: { position: 'absolute', width: '100%', height: '100%', opacity: 0.6 },
  heroImageOverlay: { position: 'absolute', width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.4)' },
  avatar: { width: 100, height: 100, borderRadius: 50, zIndex: 5, borderWidth: 3, borderColor: tokens.theme.colors.surfaceBg },
  headerOverlay: { position: 'absolute', top: 40, left: 16, zIndex: 10 },
  content: { padding: tokens.theme.spacing.lg, alignItems: 'center' },
  title: { marginBottom: 16, textAlign: 'center' },
  statsRow: { flexDirection: 'row', gap: 12, marginBottom: 24, justifyContent: 'center' },
