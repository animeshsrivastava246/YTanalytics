...
We define a set of core presentational layer components. These components do *not* contain business logic, data fetching, or global state.
...
export const GlassSurface = ({ type = 'secondary', children, style, ...rest }) => {
...
export const PrimaryButton = ({ label, onPress, variant = 'solid' }) => {
  ...
  return (
    <Animated.View style={[stylez]}>
      ...
      >
        <AppText variant="subtitle" color={variant === 'solid' ? 'primary' : 'muted'}>
          {label}
        </AppText>
      </Pressable>
    </Animated.View>
