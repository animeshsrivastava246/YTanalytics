...
- **Results List**: `FlashList` (or `FlatList`) showing results.
  - _Video Row_: Thumbnail on the left, Title and Channel name on the right. Overlaid `StatPill` on bottom right of thumbnail for duration.
  - _Channel Row_: Circular avatar thumbnail on the left, Channel Name and subscriber count on right.
...
- **Aggregate Time Block**: Similar to Video Detail, but calculates the watch time for the _entire_ playlist. Uses the Phase 6 `useWatchTime` hook passing the list of playlist items.
